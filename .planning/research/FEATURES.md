...
|---------|--------------|------------|-------|
| Infinite pan and zoom | Core UX paradigm; Excalidraw/tldraw/Mesa all have it | Medium | Collaborator uses 33-100% range. Consider wider range (10-200%). |
...
| Minimap / overview | Navigation aid for large canvases; Excalidraw has this | Medium | Essential once users have 10+ tiles spread across canvas |
...
|---------|-------------------|------------|-------|
| Tile snapping / alignment guides | Excalidraw-style smart snapping when dragging tiles near each other. Collaborator issue #23. | Medium | Show alignment lines when edges/centers align with neighbors |
...
| Cloud sync / collaboration | Massively increases complexity (auth, conflict resolution, servers). Miro/Excalidraw territory. | Local-only persistence. Export/import canvas state for sharing. |
